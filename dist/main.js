...
var GeneralSection = class {
  ...
  async checkDependencies() {
    ...
    for (const plugin of JOTS_PLUGINS) {
      const id = plugin.repo.split("/")[1];
      await checkPlugin(id);
    }
  }
  async display(containerEl) {
    await this.checkDependencies();
    new import_obsidian.Setting(containerEl).setName("Auto-update plugins at startup").setDesc("If enabled, installed plugins will be checked for updates each time Obsidian starts").addToggle((toggle) => toggle.setValue(this.plugin.settings.updateAtStartup).onChange(async (value) => {
  ...
  async createDependencyControls(containerEl, name, desc, id, repo) {
    var _a, _b;
    ...
    Promise.all([
      ...
      var _a2;
      const isInstalled2 = (_a2 = this.dependencyState[id]) == null ? void 0 : _a2.isInstalled;
      const versionNumber = isInstalled2 ? installedVer || "N/A" : latestVer || "N/A";
    ...
    const isInstalled = (_a = this.dependencyState[id]) == null ? void 0 : _a.isInstalled;
    const isLoaded = ((_b = this.app.plugins) == null ? void 0 : _b.plugins[id]) !== void 0;
    const isEnabled = isInstalled && isLoaded;
    if (isEnabled) {
      setting.addButton(
        (btn) => btn.setIcon("gear").setTooltip(`Open ${name} settings`).onClick(async () => {
    ...
    setting.addButton((btn) => {
      if (!isInstalled) {
        btn.setButtonText("Install").setCta().onClick(async () => {
          ...
          if (success && (state == null ? void 0 : state.isInstalled)) {
            btn.setButtonText("Uninstall").removeCta().setDisabled(state.isEnabled);
            setting.addToggle((toggle) => {
              toggle.setValue(state.isEnabled).setTooltip(state.isEnabled ? `Disable ${name}` : `Enable ${name}`).onChange(async (value) => {
                var _a2;
                if (value)
                  await this.app.plugins.enablePluginAndSave(id);
                else
                  await this.app.plugins.disablePluginAndSave(id);
                await this.checkDependencies();
                const newState = this.dependencyState[id];
                btn.setDisabled((_a2 = newState == null ? void 0 : newState.isEnabled) != null ? _a2 : false);
              });
            });
          } else {
            btn.setButtonText("Install").setCta().setDisabled(false);
          }
        });
      } else {
        btn.setButtonText("Uninstall").setDisabled(isEnabled).setTooltip("Disable plugin before Uninstalling").onClick(async () => {
          btn.setButtonText("Uninstalling...");
          btn.setDisabled(true);
          await this.plugin.pluginManager.uninstallPlugin(id);
          await this.checkDependencies();
          const state = this.dependencyState[id];
          if (!(state == null ? void 0 : state.isInstalled)) {
            btn.setButtonText("Install").setCta().setDisabled(false);
            setting.components = setting.components.filter((c) => c instanceof import_obsidian.Setting);
          }
        });
        setting.addToggle((toggle) => {
          toggle.setValue(isEnabled).setTooltip(isEnabled ? `Disable ${name}` : `Enable ${name}`).onChange(async (value) => {
            var _a2;
            if (value)
              await this.app.plugins.enablePluginAndSave(id);
            else
...
var AppearanceSection = class {
  ...
  display(containerEl) {
    ...
    containerEl.createEl("hr");
    new import_obsidian2.Setting(containerEl).setName("JOTS Section Name").setDesc("The name of the callout/section that will contain your Jots").addText((text) => text.setPlaceholder("JOTS").setValue(this.plugin.settings.sectionName).onChange(async (value) => {
      this.plugin.settings.sectionName = value;
      await this.plugin.saveSettings({ refreshType: "styles" });
    }));
    new import_obsidian2.Setting(containerEl).setName("Label Color").setDesc("The color of the JOTS callout/section label").addColorPicker((color) => color.setValue(this.plugin.settings.labelColor).onChange(async (value) => {
      this.plugin.settings.labelColor = value;
      await this.plugin.saveSettings({ refreshType: "styles" });
    }));
    new import_obsidian2.Setting(containerEl).setName("JOTS Section Icon").setDesc("SVG icon data string for the callout/section (must be valid SVG XML)").addTextArea((text) => text.setPlaceholder("<svg>...</svg>").setValue(this.plugin.settings.sectionIcon).onChange(async (value) => {
      this.plugin.settings.sectionIcon = value;
      await this.plugin.saveSettings({ refreshType: "styles" });
    }));
    new import_obsidian2.Setting(containerEl).setName("JOTS Section Format").setDesc("How the JOTS callout/section should behave").addDropdown((dropdown) => dropdown.addOption("Plain", "Plain").addOption("Foldable-Open", "Foldable (Open by default)").addOption("Foldable-Closed", "Foldable (Closed by default)").setValue(this.plugin.settings.sectionFormat).onChange(async (value) => {
      this.plugin.settings.sectionFormat = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Task Letters").setDesc('Letters to look for in custom tasks (comma-separated, e.g. "A,B,C"). The JOTS header will only be added to notes containing tasks with these letters. Case-insensitive.').addText((text) => text.setPlaceholder("A,B,C").setValue(this.plugin.settings.taskLetters.join(",")).onChange(async (value) => {
      let normalized = value.toUpperCase();
      const validLetters = normalized.split(",").map((letter) => letter.trim()).filter((letter) => /^[A-Z]$/.test(letter));
...
var JournalSection = class {
  ...
  display(containerEl) {
    ...
    containerEl.createEl("hr");
    new import_obsidian3.Setting(containerEl).setName("Journal Root Folder").setDesc("The root folder where journals are stored").addText((text) => text.setPlaceholder("Journals").setValue(this.plugin.settings.journalRootFolder).onChange(async (value) => {
      this.plugin.settings.journalRootFolder = value;
      await this.plugin.saveSettings({ refreshViews: false });
    }));
    new import_obsidian3.Setting(containerEl).setName("Journal Folder Pattern").setDesc("Pattern for journal folder structure (using moment.js format)").addText((text) => text.setPlaceholder("YYYY/YYYY-MM").setValue(this.plugin.settings.journalFolderPattern).onChange(async (value) => {
      this.plugin.settings.journalFolderPattern = value;
      await this.plugin.saveSettings({ refreshViews: false });
    }));
    new import_obsidian3.Setting(containerEl).setName("Journal File Pattern").setDesc("Pattern for journal file names (using moment.js format)").addText((text) => text.setPlaceholder("YYYY-MM-DD_ddd").setValue(this.plugin.settings.journalFilePattern).onChange(async (value) => {
...
var MultiSuggest = class extends import_obsidian4.AbstractInputSuggest {
...
// src/settings/sections/HeadersFootersSection.ts
var HeadersFootersSection = class {
  constructor(plugin, app) {
    this.plugin = plugin;
    this.app = app;
  ...
  getAvailableFolderPaths() {
    ...
    files.forEach((file) => {
      var _a;
      let path = (_a = file.parent) == null ? void 0 : _a.path;
      while (path && path !== "/") {
        paths.add(path + "/");
  ...
  getAvailableTags() {
    ...
    this.plugin.app.vault.getMarkdownFiles().forEach((file) => {
      ...
      if (fileCache) {
        const fileTags = (0, import_obsidian5.getAllTags)(fileCache) || [];
  ...
  display(containerEl) {
    ...
    containerEl.createEl("hr");
    new import_obsidian5.Setting(containerEl).setName("Refresh on file open").setDesc("Refresh headers and footers when opening a note").addToggle((toggle) => {
      var _a;
      return toggle.setValue((_a = this.plugin.settings.refreshOnFileOpen) != null ? _a : false).onChange(async (value) => {
    ...
    new import_obsidian5.Setting(containerEl).addButton((button) => button.setButtonText("Add new rule").setCta().onClick(async () => {
      const newRule = {
        name: "New Rule",
        enabled: true,
        type: "folder" /* Folder */,
  ...
  renderRuleControls(rule, index, containerEl) {
    ...
    const ruleNameDisplay = ((_a = rule.name) == null ? void 0 : _a.trim()) || "Unnamed Rule";
    ...
    new import_obsidian5.Setting(ruleContent).setName("Rule name").setDesc("A descriptive name for this rule").addText((text) => text.setPlaceholder("e.g., Project Notes Footer").setValue(rule.name || "").onChange(async (value) => {
      rule.name = value;
      ruleHeading.textContent = `Rule ${index + 1}: ${value.trim() || "Unnamed Rule"}`;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(ruleContent).setName("Enabled").setDesc("If disabled, this rule will not be applied").addToggle((toggle) => {
      var _a2;
      return toggle.setValue((_a2 = rule.enabled) != null ? _a2 : true).onChange(async (value) => {
        rule.enabled = value;
        await this.plugin.saveSettings({ refreshType: "content" });
      });
    });
    new import_obsidian5.Setting(ruleContent).setName("Rule type").setDesc("Apply this rule based on folder, tag, or property").addDropdown((dropdown) => dropdown.addOption("folder" /* Folder */, "Folder").addOption("tag" /* Tag */, "Tag").addOption("property" /* Property */, "Property").setValue(rule.type).onChange(async (value) => {
      rule.type = value;
      await this.plugin.saveSettings({ refreshType: "content" });
      this.display(containerEl);
    }));
    if (rule.type === "folder" /* Folder */) {
      new import_obsidian5.Setting(ruleContent).setName("Folder path").setDesc('Path for the rule. Use "" for all files, "/" for root folder').addText((text) => {
        text.setPlaceholder("e.g., Projects/ or / or empty for all").setValue(rule.path || "").onChange(async (value) => {
          rule.path = value;
          await this.plugin.saveSettings();
        });
        new MultiSuggest(
      ...
      new import_obsidian5.Setting(ruleContent).setName("Include subfolders").setDesc("If enabled, rule applies to files in subfolders").addToggle((toggle) => {
        var _a2;
        return toggle.setValue((_a2 = rule.recursive) != null ? _a2 : true).setDisabled(rule.path === "").onChange(async (value) => {
          rule.recursive = value;
          await this.plugin.saveSettings();
        });
      });
    } else if (rule.type === "tag" /* Tag */) {
      new import_obsidian5.Setting(ruleContent).setName("Tag").setDesc("Tag to match (without #)").addText((text) => {
        text.setPlaceholder("e.g., project or status/done").setValue(rule.tag || "").onChange(async (value) => {
      ...
      new import_obsidian5.Setting(ruleContent).setName("Include subtags").setDesc("If enabled, matches subtags (e.g., project/subtag)").addToggle((toggle) => {
        var _a2;
        return toggle.setValue((_a2 = rule.includeSubtags) != null ? _a2 : false).onChange(async (value) => {
          rule.includeSubtags = value;
          await this.plugin.saveSettings();
        });
      });
    } else if (rule.type === "property" /* Property */) {
      new import_obsidian5.Setting(ruleContent).setName("Property name").setDesc("The frontmatter property to match").addText((text) => {
        text.setPlaceholder("e.g., status or type").setValue(rule.propertyName || "").onChange(async (value) => {
      ...
      new import_obsidian5.Setting(ruleContent).setName("Property value").setDesc("The value the property should have").addText((text) => text.setPlaceholder("e.g., complete or draft").setValue(rule.propertyValue || "").onChange(async (value) => {
        rule.propertyValue = value;
        await this.plugin.saveSettings();
      }));
    }
    new import_obsidian5.Setting(ruleContent).setName("Content source").setDesc("Where to get the content from").addDropdown((dropdown) => dropdown.addOption("text" /* Text */, "Direct text").addOption("file" /* File */, "Markdown file").setValue(rule.contentSource).onChange(async (value) => {
      rule.contentSource = value;
      await this.plugin.saveSettings();
      this.display(containerEl);
    }));
    if (rule.contentSource === "file" /* File */) {
      new import_obsidian5.Setting(ruleContent).setName("Content file").setDesc("The markdown file to use as content").addText((text) => {
        text.setPlaceholder("e.g., templates/footer.md").setValue(rule.footerFilePath || "").onChange(async (value) => {
      ...
      new import_obsidian5.Setting(ruleContent).setName("Content").setDesc("The markdown content to insert").addTextArea((text) => text.setPlaceholder("Enter markdown content...").setValue(rule.footerText).onChange(async (value) => {
        rule.footerText = value;
        await this.plugin.saveSettings();
      }));
    }
    new import_obsidian5.Setting(ruleContent).setName("Render location").setDesc("Where to insert the content").addDropdown((dropdown) => dropdown.addOption("footer" /* Footer */, "Footer").addOption("header" /* Header */, "Header").setValue(rule.renderLocation).onChange(async (value) => {
      rule.renderLocation = value;
      await this.plugin.saveSettings({ refreshType: "content" });
    }));
    const ruleActions = new import_obsidian5.Setting(ruleContent).setClass("virtual-footer-rule-actions");
    ruleActions.addButton((button) => button.setIcon("arrow-up").setTooltip("Move rule up").setDisabled(index === 0).onClick(async () => {
      if (index > 0) {
    ...
    ruleActions.addButton((button) => button.setIcon("arrow-down").setTooltip("Move rule down").setDisabled(index === this.plugin.settings.rules.length - 1).onClick(async () => {
    ...
    ruleActions.addButton((button) => button.setIcon("trash").setTooltip("Delete rule").onClick(async () => {
...
// src/settings.ts
var JotsSettingTab = class extends import_obsidian6.PluginSettingTab {
...
// src/commands/addJotsRange.ts
var import_obsidian9 = require("obsidian");

// src/commands/dateRangeModal.ts
var import_obsidian8 = require("obsidian");
var import_moment = __toESM(require_moment());
var DateRangeModal = class extends import_obsidian8.Modal {
  constructor(app, onSubmit) {
    super(app);
    this.startDate = (0, import_moment.default)().startOf("day");
    this.endDate = (0, import_moment.default)().startOf("day");
    this.onSubmit = onSubmit;
  }
  onOpen() {
    const { contentEl } = this;
    contentEl.createEl("h3", { text: "Select Date Range" });
    new import_obsidian8.Setting(contentEl).setName("Start Date").addText((text) => {
      const input = text.inputEl;
      input.type = "date";
      input.value = this.startDate.format("YYYY-MM-DD");
      input.onchange = () => {
        const date = (0, import_moment.default)(input.value, "YYYY-MM-DD", true);
        if (date.isValid()) {
          this.startDate = date;
        }
      };
    ...
    new import_obsidian8.Setting(contentEl).setName("End Date").addText((text) => {
      const input = text.inputEl;
      input.type = "date";
    ...
    new import_obsidian8.Setting(contentEl).addButton((btn) => btn.setButtonText("Submit").setCta().onClick(() => {
...
// src/commands/addJotsRange.ts
var AddJotsRangeCommand = class {
  constructor(app, settings, addJotsCommand) {
    this.id = "add-jots-to-journal-range";
    this.name = "Add JOTS to Journal Range";
    this.app = app;
...
async function openDateRangeModal(app) {
  return new Promise((resolve) => {
    const modal = new DateRangeModal(app, (range) => {
      resolve(range);
    });
    modal.open();
  });
...
// src/commands/refreshHeadersFooters.ts
var RefreshHeadersFootersCommand = class {
  constructor(plugin) {
    this.plugin = plugin;
    this.id = "refresh-headers-footers";
    this.name = "Refresh headers and footers";
    this.callback = () => {
...
var PluginManager = class {
  ...
  async addPlugin(repositoryPath) {
    try {
      ...
      if (currentVersion) {
        ...
        new import_obsidian10.Notice(`Plugin ${manifest.name} has been installed`);
      }
      return true;
    } catch (error) {
      console.error("Error installing plugin:", error);
      new import_obsidian10.Notice(`Failed to install plugin: ${error.message}`);
      return false;
    }
  }
  ...
  async validateRepository(repositoryPath) {
    try {
      ...
      const manifestContent = await this.fetchReleaseFile(release, "manifest.json", headers);
      if (!manifestContent) {
        new import_obsidian10.Notice("No manifest.json found in release");
        return null;
      }
      let manifest;
      try {
        ...
        new import_obsidian10.Notice("Invalid manifest.json format");
        return null;
      }
      if (!manifest.id || !manifest.version) {
        new import_obsidian10.Notice("Invalid manifest.json (missing id or version)");
        return null;
      }
      return manifest;
    } catch (error) {
      ...
      if (error.status === 403) {
        ...
        new import_obsidian10.Notice("Invalid GitHub personal access token. Please check your token in settings.");
      }
      return null;
    }
  }
  ...
  async fetchReleaseFile(release, fileName, headers) {
    try {
      ...
      new import_obsidian10.Notice(`Error fetching ${fileName}: ${error.message}`);
      return null;
    }
  }
  async getReleaseFiles(repositoryPath, manifest) {
    try {
      ...
      if (error.status === 403 || error.status === 401) {
        new import_obsidian10.Notice(`Failed to get release files: ${error.message}`);
      }
      return null;
    }
  }
  ...
  async enablePlugin(pluginId) {
    try {
      ...
      new import_obsidian10.Notice(`Failed to enable plugin: ${error.message}`);
      throw error;
    }
  }
  async uninstallPlugin(pluginId) {
    try {
      ...
      if (!manifest) {
        new import_obsidian10.Notice(`Plugin ${pluginId} not found`);
        return false;
      }
      const pluginDir = `${app.vault.configDir}/plugins/${pluginId}/`;
      const { adapter } = app.vault;
      if (!await adapter.exists(pluginDir)) {
        new import_obsidian10.Notice(`Plugin directory not found: ${pluginId}`);
        return false;
      }
      ...
      new import_obsidian10.Notice(`Plugin ${manifest.name} has been uninstalled`);
      return true;
    } catch (error) {
      console.error("Error uninstalling plugin:", error);
      new import_obsidian10.Notice(`Failed to uninstall plugin: ${error.message}`);
      return false;
    }
  }
  async installPlugin(name, desc, repo, pluginId) {
    try {
      ...
      const { adapter } = app.vault;
      if (await adapter.exists(pluginDir)) {
        new import_obsidian10.Notice(`Plugin ${name} is already installed`);
        return false;
      }
      try {
        await adapter.mkdir(pluginDir);
        ...
        new import_obsidian10.Notice(`Failed to create plugin directory: ${error.message}`);
        return false;
      }
      if (!await adapter.exists(pluginDir)) {
        new import_obsidian10.Notice("Failed to create plugin directory");
        return false;
      }
      try {
        const manifestJson = {
      ...
      if (!mainJsExists || !manifestExists) {
        await adapter.rmdir(pluginDir, true);
        new import_obsidian10.Notice("Failed to verify plugin files after installation");
        return false;
      }
      await app.plugins.loadManifests();
      new import_obsidian10.Notice(`Plugin ${name} has been installed`);
      return true;
    } catch (error) {
      console.error("Error installing plugin:", error);
      new import_obsidian10.Notice(`Failed to install plugin: ${error.message}`);
...
var CSS_CM_PADDING = "jots-cm-padding";
...
var ContentRenderer = class {
  ...
  async renderMarkdown(content) {
    ...
    await import_obsidian11.MarkdownRenderer.renderMarkdown(
      content,
      container,
  ...
  injectContent(headerContent, footerContent) {
    ...
    console.debug(
      "JOTS Assistant: Injecting content -",
      "Mode:",
      isPreviewMode ? "preview" : "source",
      "Header:",
      !!headerContent,
      "Footer:",
      !!footerContent
...
// src/core/ViewEventManager.ts
var ViewEventManager = class {
  constructor() {
    this.observers = [];
  ...
  handleDOMChange(view) {
  }
  adjustEditorPadding(contentDOM) {
...
var JotsApi = class {
   ...
   */
  refreshHeadersAndFooters() {
    this.plugin.refreshAllViews();
  }
};

// src/main.ts
var JotsPlugin = class extends import_obsidian12.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    console.debug("JOTS Assistant: API exposed on window object:", {
      api: (_a = window.JotsAssistant) == null ? void 0 : _a.api,
    ...
    const leafFiles = /* @__PURE__ */ new WeakMap();
  ...
  async checkForUpdates() {
    ...
    for (const pluginId of managedPlugins) {
      ...
      try {
        ...
        if (result) {
          new import_obsidian12.Notice(`Plugin ${manifest.name} has been updated to the latest version`);
        }
      } catch (error) {
  ...
  async _processView(view) {
    ...
    const tryInjectContent = (attempt = 1) => {
